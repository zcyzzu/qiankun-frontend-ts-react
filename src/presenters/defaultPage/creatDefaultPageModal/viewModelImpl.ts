 ...
 * @LastEditors: wuhao
 * @LastEditTime: 2022-04-21 14:38:28
 */
...
import CreatPublishDafaultPageModalViewModel, { FormData } from './viewModel';
...
@injectable()
export default class CreatDafaultPageModalViewModelImpl
  ...
  public constructor() {
    ...
    makeObservable(this, {
      ...
      getMaterialDataDefaultPlatform: action,
    });
  }
  // 获取详情
  public getDetailDefault = async (id: number): Promise<boolean> => {
    const data = await this.defaultPageUseCase.getPlatformDetailDefault(id);
    runInAction(async () => {
  ...
  public uploadChange = async (info: UploadChangeParam): Promise<void> => {
    ...
    if (status === 'done') {
      ...
      const id = await this.defaultPageUseCase.setCreateMaterial(urlParam);
      if (id) {
        this.uploading = false;
      }
      // 存id，保存或者发布需要
  ...
  public initBackgroundItem = async (): Promise<void> => {
    this.isChange = false;
    // 不要删除接口，不需要删除素材本身
    // await this.defaultPageUseCase.delMaterial(this.materialId);
    // utils.globalMessge({
    //   content: '删除素材成功',
    //   type: 'success',
    // });
  ...
  public formOnFinish = async (
    values: FormData,
    defaultPageListViewModel: DefaultPageListViewModel,
  ): Promise<void> => {
    const { deviceType, time, resolution } = values;
    // 创建缺省
    ...
    this.detailData.unitId = 0;
    this.detailData.unitName = '';
    this.detailData.tenantId = 0;
    if (this.modalType === ModalStatus.Creat) {
      this.addNewAdvert(param, defaultPageListViewModel);
    } else {
      this.putNewAdvert(this.detailData, defaultPageListViewModel);
    }

    // const id = await this.defaultPageUseCase.setCre
    // ateDefault(param, publishDefaultPageListViewModel);
  };
  ...
  public addNewAdvert = (
    ...
    this.defaultPageUseCase
      .setPlatformCreateDefault(param)
      .then(() => {
        ...
        if (defaultPageListViewModel) {
          const { getDefaultPageList } = defaultPageListViewModel;
          getDefaultPageList();
        }
      })
      .catch((error) => {
  ...
  // 获取选中的素材数据
  public getMaterialDataDefaultPlatform = async (
