 ...
 * @LastEditors: wuhao
 * @LastEditTime: 2022-02-25 16:07:24
 */
import { AxiosError, AxiosInstance, AxiosRequestConfig, AxiosResponse } from 'axios';
import DI from '../inversify.config';
import ConfigProvider from '../common/config/configProvider';
import { ROOT_CONTAINER_IDENTIFIER, CONFIG_IDENTIFIER } from '../constants/identifiers';
...
const onRequest = (config: AxiosRequestConfig): AxiosRequestConfig => {
  ...
  // if ((newConfig.url?.includes('/tmis') && !newConfig.url?.includes('/site')) || 
...
const onResponseError = (error: AxiosError): Promise<AxiosError> => {
  ...
  if (error.response?.status === 403) {
    history.push('/tmis/notPass');
  }
  if (error.response?.status === 404) {
    history.push('/tmis/notFound');
  }
  if (
    error.response?.status === 401 ||
    (error.response?.status === 403 &&
      (error.response?.data as string).includes('error.permission.accessTokenExpired'))
  ) {
    const encodedLoginRedirectUri = encodeURIComponent(`${window.location.href}&redirectFlag`);
